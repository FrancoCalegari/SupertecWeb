...
document.addEventListener("DOMContentLoaded", () => {
	...
	const whatsappNumber = "5492615031101";
						...
						card.innerHTML = `
                            ...
                            ${
                                    ...
                                    <span class="badge-descuento">-${
																...
																: `<p class="precio">Desde $${producto.precio.toLocaleString(
																		"es-AR"
																  )}</p>`
														}
		...
		apiFetch("/api/ventas")
			...
			.then((ventas) => {
				...
				ventas.forEach((venta) => {
					...
					card.querySelector(".btn-comprar").addEventListener("click", () => {
						// 2. ELIMINADO: El precio dentro del string del mensaje
						const mensaje = `SuperTec Web - Quisiera consultar sobre este producto del local:\n\n *${
							venta.name
						}*\n Marca: ${venta.marca}\n Modelo: ${
							venta.modelo
						}`;
			...
			.then((servicios) => {
				...
				servicios.forEach((servicio) => {
					...
					if (servicio.precio && servicio.precio > 0) {
						priceHtml = `<p class="precio">Desde $${servicio.precio.toLocaleString(
							"es-AR"
						)}</p>`;
					}
					...
					// 🔹 Evento botón WhatsApp
					card.querySelector(".btn-comprar").addEventListener("click", () => {
						let mensaje = `SuperTec Web - Quisiera Consultar por el servicio de: ${servicio.name}`;
						if (servicio.precio && servicio.precio > 0) {
